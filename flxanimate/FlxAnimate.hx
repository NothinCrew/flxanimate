...
class FlxAnimate extends FlxSprite
{
	...
	public function playShitLmfao(?Name:String = "", ?Force:Bool = false, ?Reverse:Bool = false, ?Frame:Int = 0) {
		if(anim.curInstance == null || anim.curSymbol == null) return;
		anim.play();
	}

	public function pauseShitLmfao(?Name:String = "", ?Force:Bool = false, ?Reverse:Bool = false, ?Frame:Int = 0) {
		if(anim.curInstance == null || anim.curSymbol == null) return;
		anim.pause();
	}
